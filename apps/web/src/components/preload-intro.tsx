...
// sanity-image lib renders 2 imgs (LQIP base64 + real). LQIP carries
// `data-lqip="true"` and is `complete` instantly — skip it and wait for the
// real image. Demo path uses next/image (one img, no LQIP).
...
export function PreloadIntro({
  ...
  useEffect(() => {
    ...
    // Hard cap: if kickoff or measure can't finish in time (network hang,
    // missing #hero, fonts.ready stall on Safari), bail out so the user
    // isn't stuck on a static overlay forever.
  ...
  // Solid cream bg sits behind the image so a slow image load shows the
  // backdrop colour rather than a broken-image box / page-bleed-through.
  if (!viewport) {
    return (
      <div
        aria-hidden
